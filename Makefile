...
#	cd backend && swagger generate server --principal=models.User --template=stratoscale --target=./generated --spec=../swagger.json --exclude-main --api-package=gaollery
	...
	cd backend && oapi-codegen --config=api/api.server.yaml ../openapi.json
	cd backend && oapi-codegen --config=api/api.models.yaml ../openapi.json
	cd backend && oapi-codegen --config=api/api.swagger.yaml ../openapi.json
